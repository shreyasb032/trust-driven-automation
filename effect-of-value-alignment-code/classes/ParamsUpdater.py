...
class Estimator:
    ...
    def getGrads(self, params):
        ...
        alpha_slope = -(prior["AlphaValues"][alpha_idx] - prior["AlphaValues"][max(alpha_idx - 1, 0)])/(prior["AlphaEdges"][0] - prior["AlphaEdges"][1])

        beta_idx = np.sum(params[1] > prior["BetaEdges"]) - 1
        beta_prior = prior["BetaValues"][beta_idx]
        beta_slope = -(prior["BetaValues"][beta_idx] - prior["BetaValues"][max(beta_idx - 1, 0)])/(prior["BetaEdges"][0] - prior["BetaEdges"][1])

        ws_idx = np.sum(params[2] > prior["wsEdges"]) - 1
        ws_prior = prior["wsValues"][ws_idx]
        ws_slope = -(prior["wsValues"][ws_idx] - prior["wsValues"][max(ws_idx - 1, 0)])/(prior["wsEdges"][0] - prior["wsEdges"][1])

        wf_idx = np.sum(params[3] > prior["wfEdges"]) - 1
        wf_prior = prior["wfValues"][wf_idx]
        wf_slope = -(prior["wfValues"][wf_idx] - prior["wfValues"][max(wf_idx - 1, 0)])/(prior["wfEdges"][0] - prior["wfEdges"][1])
