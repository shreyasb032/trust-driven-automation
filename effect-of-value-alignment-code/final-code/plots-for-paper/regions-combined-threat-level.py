...
def analyze(parent_directory: str, ax: Axes3D, threat_level, cmap: str='plasma'):
...
def main():
    ...
    ax1 = analyze(parent_directory, ax1, 0.7, cmap_str)

    ax2 = fig.add_subplot(122, projection='3d')
    parent_directory = path.join("..", "varying-weights", "data", "BoundedRational", "MidInitialTrust", "0.3")
    ax2 = analyze(parent_directory, ax2, 0.3, cmap_str)
    ...
    sm.set_array([])
    cb = fig.colorbar(sm, ax=[ax1, ax2], location='left', shrink=0.6)
    cb.ax.set_title("scale")

    fig.suptitle("End-of-mission Trust")
