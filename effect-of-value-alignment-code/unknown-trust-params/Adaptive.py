...
class AdaptiveRobot:
    ...
    def run_one_simulation(self, seed: int):
        ...
        solver = SolverConstantRewards(N, rob_weights, trust_params.copy(), None, None, None, est_human_weights, hum_mod='bounded_rational', reward_fun=reward_fun, hl=args.hl, tc=args.tc)
    ...
    def run(self):
        ...
        data_direc = "./data/adaptive-learner/" + time.strftime("%Y%m%d-%H%M%S") + '/' # Storage directory for the plots
        #################################################################################################################################
