...
class NonAdaptiveRobot:
    ...
    def run_one_simulation(self, seed: int):
        ...
        # CONSTANT_THREATS_FLAG = args.use_constant_threats  # Flag to set whether there is any randomization in threat levels or not (default: True)
        #################################################################################################################################
    ...
    def run(self, data_direc):
        ...
        #wh_hum = args.health_weight_human           # True health weight of the human. time weight = 1 - health weight
        #kappa = args.kappa                          # Assumed rationality coefficient in the bounded rationality model
        # data_direc = "./data/ThreatLevel/{:1.1f}/non-adaptive-learner/".format(args.threat_level) + time.strftime("%Y%m%d-%H%M%S") + '/' # Storage directory for the data
        #################################################################################################################################
